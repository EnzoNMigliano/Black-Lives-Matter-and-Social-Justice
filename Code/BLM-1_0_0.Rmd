...
NY_StopFrisk_2015 <- 
  read_csv("https://media.githubusercontent.com/media/EnzoNMigliano/Analytics-Dream-Team/main/Data/BLM%20Diverse%20Data/NYC_stop_frisk/sqf-2015.csv", cols(
  year = col_date(),
  pct = col_integer(),
  ser_num = col_number(),
  datestop = col_date(),
  timestop = col_time(),
  recstat = col_character(),
  inout = col_character(),
  trhsloc = col_character(),
  perobs = col_double(),
  crimsusp = col_factor(),
  perstop = col_character(),
  typeofid = col_character(),
  explnstp = col_logical(),
  othpers = col_character(),
  arstmade = col_character(),
  arstoffn = col_character(),
  sumissue = col_character(),
  sumoffen = col_character(),
  compyear = col_double(),
  comppct = col_double(),
  offunif = col_logical(),
  officrid = col_character(),
  frisked = col_logical(),
  searched = col_logical(),
  contrabn = col_logical(),
  adtlrept = col_character(),
  pistol = col_character(),
  riflshot = col_character(),
  asltweap = col_character(),
  knifcuti = col_character(),
  machgun = col_character(),
  othrweap = col_character(),
  pf_hands = col_logical(),
  pf_wall = col_character(),
  pf_grnd = col_character(),
  pf_drwep = col_character(),
  pf_ptwep = col_character(),
  pf_baton = col_character(),
  pf_hcuff = col_logical(),
  pf_pepsp = col_character(),
  pf_other = col_character(),
  radio = col_character(),
  ac_rept = col_character(),
  ac_inves = col_character(),
  rf_vcrim = col_character(),
  rf_othsw = col_character(),
  ac_proxm = col_character(),
  rf_attir = col_character(),
  cs_objcs = col_character(),
  cs_descr = col_character(),
  cs_casng = col_character(),
  cs_lkout = col_character(),
  rf_vcact = col_character(),
  cs_cloth = col_character(),
  cs_drgtr = col_character(),
  ac_evasv = col_character(),
  ac_assoc = col_character(),
  cs_furtv = col_character(),
  rf_rfcmp = col_character(),
  ac_cgdir = col_character(),
  rf_verbl = col_character(),
  cs_vcrim = col_character(),
  cs_bulge = col_character(),
  cs_other = col_character(),
  ac_incid = col_character(),
  ac_time = col_character(),
  rf_knowl = col_character(),
  ac_stsnd = col_character(),
  ac_other = col_character(),
  sb_hdobj = col_character(),
  sb_outln = col_character(),
  sb_admis = col_character(),
  sb_other = col_character(),
  repcmd = col_double(),
  revcmd = col_double(),
  rf_furt = col_character(),
  rf_bulg = col_character(),
  offverb = col_character(),
  offshld = col_character(),
  forceuse = col_logical(),
  sex = col_factor(),
  race = col_factor(),
  dob = col_logical(),
  age = col_integer(),
  ...
  ht_inch = col_double(),
  weight = col_integer(),
  haircolr = col_character(),
  eyecolor = col_character(),
  build = col_factor(),
  othfeatr = col_character(),
  addrtyp = col_character(),
  rescode = col_logical(),
  premtype = col_logical(),
  premname = col_character(),
  addrnum = col_character(),
  stname = col_character(),
  stinter = col_character(),
  crossst = col_character(),
  aptnum = col_logical(),
  city = col_factor(),
  state = col_logical(),
  zip = col_logical(),
...
```{r}
NY_StopFrisk_2018Tidy <- NY_StopFrisk_2018 %>% subset(select = c(YEAR2, STOP_FRISK_DATE, SUSPECTED_CRIME_DESCRIPTION, OFFICER_EXPLAINED_STOP_FLAG, 
                                                                  OFFICER_IN_UNIFORM_FLAG, FRISKED_FLAG, SEARCHED_FLAG, OTHER_CONTRABAND_FLAG,
                                                                 PHYSICAL_FORCE_HANDCUFF_SUSPECT_FLAG, PHYSICAL_FORCE_RESTRAINT_USED_FLAG, SUSPECT_SEX, SUSPECT_RACE_DESCRIPTION, 
                                                                 SUSPECT_REPORTED_AGE, SUSPECT_HEIGHT, SUSPECT_WEIGHT, SUSPECT_BODY_BUILD_TYPE, STOP_LOCATION_BORO_NAME)) %>% rename(Year = YEAR2,`Date of Frisk` = STOP_FRISK_DATE,`Crime Suspected` = SUSPECTED_CRIME_DESCRIPTION, `Officer Explained Reason` = OFFICER_EXPLAINED_STOP_FLAG, `Officer Wearing Uniform` = OFFICER_IN_UNIFORM_FLAG, Frisked = FRISKED_FLAG, `Body Searched` = SEARCHED_FLAG, Contraband = OTHER_CONTRABAND_FLAG, `Hanfcuff Used` = PHYSICAL_FORCE_HANDCUFF_SUSPECT_FLAG, `Force Used` = PHYSICAL_FORCE_RESTRAINT_USED_FLAG, Sex = SUSPECT_SEX, Race = SUSPECT_RACE_DESCRIPTION, Age = SUSPECT_REPORTED_AGE, Height = SUSPECT_HEIGHT, Weight = SUSPECT_WEIGHT, `Body Type` = SUSPECT_BODY_BUILD_TYPE, City = STOP_LOCATION_BORO_NAME)
...
```{r}
view(NY_StopFrisk_2019)
```

We are subseting the data with relevant values

```{r}
NY_StopFrisk_2019Tidy <- NY_StopFrisk_2019 %>% subset(select = c(YEAR2, STOP_FRISK_DATE, SUSPECTED_CRIME_DESCRIPTION, OFFICER_EXPLAINED_STOP_FLAG, 
                                                                  OFFICER_IN_UNIFORM_FLAG, FRISKED_FLAG, SEARCHED_FLAG, OTHER_CONTRABAND_FLAG,
                                                                 PHYSICAL_FORCE_HANDCUFF_SUSPECT_FLAG, PHYSICAL_FORCE_RESTRAINT_USED_FLAG, SUSPECT_SEX, SUSPECT_RACE_DESCRIPTION, 
                                                                 SUSPECT_REPORTED_AGE, SUSPECT_HEIGHT, SUSPECT_WEIGHT, SUSPECT_BODY_BUILD_TYPE, STOP_LOCATION_BORO_NAME)) %>% rename(Year = YEAR2,`Date of Frisk` = STOP_FRISK_DATE,`Crime Suspected` = SUSPECTED_CRIME_DESCRIPTION, `Officer Explained Reason` = OFFICER_EXPLAINED_STOP_FLAG, `Officer Wearing Uniform` = OFFICER_IN_UNIFORM_FLAG, Frisked = FRISKED_FLAG, `Body Searched` = SEARCHED_FLAG, Contraband = OTHER_CONTRABAND_FLAG, `Hanfcuff Used` = PHYSICAL_FORCE_HANDCUFF_SUSPECT_FLAG, `Force Used` = PHYSICAL_FORCE_RESTRAINT_USED_FLAG, Sex = SUSPECT_SEX, Race = SUSPECT_RACE_DESCRIPTION, Age = SUSPECT_REPORTED_AGE, Height = SUSPECT_HEIGHT, Weight = SUSPECT_WEIGHT, `Body Type` = SUSPECT_BODY_BUILD_TYPE, City = STOP_LOCATION_BORO_NAME)
...
Then, the merged dataset with 2018

```{r}
NY_StopFrisk3 <- full_join(NY_StopFrisk2, NY_StopFrisk_2018Tidy)
```
```{r}
NY_StopFrisk_2018Tidy
```
...
```{r}
NY_StopFrisk_2018Tidy$`Date of Frisk` <- parse_date(NY_StopFrisk_2018Tidy$`Date of Frisk`, "%d/%m/%y")
NY_StopFrisk_2018Tidy$Age <- as.double(NY_StopFrisk_2018Tidy$Age)
